...
- `-a2a_addr` serve A2A JSON-RPC (`/invoke`) and agent card (`/.well-known/agent-card.json`)
- `-a2a_url` optional public base URL for the agent card (defaults to `http://<addr>/invoke`)
...
- A2A server: `./tumix -a2a_addr :8081`
