...
func TestXAILLM_Generate(t *testing.T) {
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			conn, cleanup := rr.NewGRPCConn(t, "xai", xai.APIHost, xai.BuildDialOptions(opts, os.Getenv("XAI_API_KEY"))...)
			t.Cleanup(cleanup)

			mgmtConn, cleanup2 := rr.NewGRPCConn(t, "xai-management", xai.ManagementAPIHost, xai.BuildDialOptions(opts, os.Getenv("XAI_MANAGEMENT_KEY"))...)
			t.Cleanup(cleanup2)

			llm, err := NewXAILLM(t.Context(), AuthMethodAPIKey("test-key"), tt.modelName,
